...
def search_hybrid(query):
    ...
    if collection:
        try:
            ...
            # 유사도 임계값 1.1 적용 (L2 거리 기준, 작을수록 유사함. 임계값을 완화하고 LLM으로 관련성 2차 검증 수행)
            ...
            # 로컬 지식이 존재하면 LLM 필터링 없이 최우선적으로 바로 활용 (지나친 필터링으로 인한 누락 방지)
            if contexts:
...
4. 제공된 [지식]이 비어있을 때만 선생님의 자체 지식으로 답변합니다. 이때는 답변 맨 앞에 반드시 "[선생님이 가진 추가 지식으로 답변해 줄게요!]" 라는 안내 문구를 출력하세요. 자체 지식으로 답변할 때에는 존재하지 않는 가짜 웹사이트 주소(URL)를 지어내는 것을 방지하기 위해, 특정 웹사이트 주소를 적지 말고 학생이 직접 추가 학습을 할 수 있는 구글 검색 링크를 제공하세요. 답변 텍스트 맨 마지막 줄에 반드시 `<source>구글 검색: [핵심 키워드]|https://www.google.com/search?q=[검색+키워드]</source>` 형식으로 적어주세요. (예: <source>구글 검색: 오세아니아 기후|https://www.google.com/search?q=오세아니아+기후</source>). 주의: 이 <source> 태그를 절대 마크다운 코드 블록(```)으로 감싸지 말고 그냥 생텍스트로 적으세요. 다른 임의의 URL은 절대 적지 마세요.
