...
def search_hybrid(query):
    ...
    if collection:
        try:
            ...
            # 유사도 임계값 1.5 적용 (1단계 교과서/지도서 로컬 지식을 최대한 활용하기 위해 완화)
...
4. 제공된 [지식]이 비어있을 때만 선생님의 자체 지식으로 답변합니다. 이때는 답변 맨 앞에 반드시 "[선생님이 가진 추가 지식으로 답변해 줄게요!]" 라는 안내 문구를 출력하세요. 자체 지식으로 답변할 때에는 '구글 검색', '출처', '인터넷 주소', 'URL', 'http', 'source' 등을 절대 적지 말고 오직 텍스트로만 자세히 설명하세요.
